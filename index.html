...
const storage={
  ...
  save(){
    localStorage.setItem('reCodeSession',JSON.stringify(this.data));
  },
  load(){
...
function App(){
  ...
  // ✅ HuggingFace AI Generation (Primary and Only)
  ...
  async function continueGeneration(){
    ...
    let buffer="";
    const systemPrompt=`You are an elite code generation expert. Generate production-ready, fully functional code with these requirements:
...
- Complete all functions, classes, and statements fully`;
    ...
    try{
      ...
      showToast("Generating code with DeepSeek Coder...","info");

      const aiResponse=await generateWithAI(systemPrompt,prompt);

      clearInterval(progressInterval);
      setProgress(95);

      buffer=aiResponse.code;
  ...
  async function continueActiveFile(){
    ...
    const systemPrompt=`You are an elite code generation expert. Continue the code with these quality standards:
...
- Ensure the continuation is production-ready`;
    ...
    try{
      ...
      setProgress(100);
      showToast(`✨ Code continued successfully! Used ${cost} credits.`,"success");
              ...
              {files.map(file=>(
                ...
                >
                  <div className="file-info">
                    ...
                    <div className="file-meta">{(file.size/1000).toFixed(1)}KB • {file.language}</div>
                  </div>
            ...
            <div className="prompt-section">
              ...
              {prompt&&(
                <div className="token-estimate">
                  ...
                  <span>•</span>
                  <span style={{color:"#6ef3c5"}}>✨ Quality assured</span>
                </div>
              )}
            </div>
          ...
          <div className="editor-container">
            ...
            <div className="stats">
              ...
              <div className="stat-item" style={{color:"#6ef3c5"}}>
                <i data-lucide="cpu"></i>
                <span>AI: <span className="stat-value">DeepSeek V2</span></span>
              </div>
            </div>
          </div>
        </div>
